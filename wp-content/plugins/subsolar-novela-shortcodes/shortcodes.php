...
if ( !function_exists('sdesigns_button') ) {
	function sdesigns_button( $atts, $content = null ) {
		...
		if ( $type == 'read' ) {
			return '<button class="button read-book-button" data-id="' . $id . '">'. do_shortcode($content) .'</button>';
		} else if ( $type = 'dotted' ) {
			return '<a href="' . $to . '" class="button dotted-button" ' . ($page == 'new' ? 'target="_blank"' : '') . '>'. do_shortcode($content) .'</a>';
		} else {
...
if ( !function_exists('sdesigns_lead') ) {
	function sdesigns_lead( $atts, $content = null ) {
		$defaults = array();
		extract( shortcode_atts( $defaults, $atts ) );

		return '<div class="lead">'. do_shortcode($content) .'</div>';
	}
	add_shortcode( 'lead', 'sdesigns_lead' );
}
...
if ( !function_exists('sdesigns_collapsibles') ) {
	function sdesigns_collapsibles( $atts, $content = null ) {
		...
		return '<div class="panel-group" id="'. esc_attr( $id ) .'">'. do_shortcode( $content ) .'</div>';
	}
